...
const Admin = ({ events }) => {
  ...
  if (loading) {
    return <p className="text-center text-lg text-gray-300">جارٍ التحميل...</p>;
  }

  const handleCreateEvent = async () => {
    if (!eventName.trim()) {
      setErrorMessage('يرجى إدخال اسم المناسبة');
      return;
    }
    try {
    ...
    <div className="min-h-screen bg-gradient-to-r from-[#141517] via-[#1e2024] to-[#3c4043] font-tajawal text-white">
      ...
      <header className="flex justify-between items-center bg-transparent backdrop-blur-md text-white p-4 sm:p-6 shadow-xl">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <h1 className="text-lg sm:text-2xl font-bold">لوحة إدارة المناسبات</h1>
        </div>
      </header>
      ...
      <section className="max-w-3xl mx-auto my-8 sm:my-16 p-6 sm:p-8 bg-[#2b2f38] rounded-lg shadow-lg backdrop-blur-md">
        <h2 className="text-2xl sm:text-3xl font-bold text-center text-[#3ab9f1] mb-6 sm:mb-8">إنشاء مناسبة جديدة</h2>
      ...
      <section className="max-w-4xl mx-auto my-8 sm:my-12 space-y-6 sm:space-y-8">
        <h2 className="text-2xl sm:text-3xl font-bold text-center text-[#3ab9f1]">المناسبات الحالية</h2>
    ...
    >
      ...
      <div className="relative z-10 space-y-3 sm:space-y-4 flex flex-col justify-between h-full">
        ...
        <div className="flex justify-between items-center text-white">
          <h3 className="text-lg sm:text-xl font-semibold text-ellipsis overflow-hidden">{event.name}</h3>
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 sm:h-8 sm:w-8 text-[#3ab9f1]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="space-y-2 sm:space-y-3 text-gray-300">
          <div className="flex justify-center items-center space-x-2">
            ...
            <span className="font-medium text-sm sm:text-base"> :باركود</span>
          </div>
          <div className="flex justify-center items-center space-x-2">
            <span className="text-sm sm:text-base">{event.access_code}</span>
            <span className="font-medium text-sm sm:text-base">:كود الوصول</span>
          </div>
        </div>
        {/* Buttons */}
